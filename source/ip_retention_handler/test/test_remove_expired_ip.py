...
def test_deserialize_ddb_data():
    ...
    expected_desiralized_ddb_ip_set = {'IPSetName': 'fake-ip-set-name', 'CreatedByUser': 'fake-user',
                                       'Scope': 'CLOUDFRONT', 'CreationTime': Decimal('1628203216'),
                                       'LockToken': 'fake-lock_token', 'IPAdressList': ['x.x.x.x/32', 'y.y.y.y/32'],
                                       'ExpirationTime': Decimal('1628203246'), 'IPSetId': 'fake-ips-set-id'}
...
def test_remove_expired_ip():
    try:
        lambda_handler(EVENT, SimpleNamespace(**{
            'function_name': 'foo',
